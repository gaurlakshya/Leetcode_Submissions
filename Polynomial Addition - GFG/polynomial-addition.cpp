...
class Solution{
  public:
    ...
    Node* addPolynomial(Node *p1, Node *p2)
    {
       ...
       Node* tmp1 = p1, *tmp2 = p2, *temp = ans;
       
       while(tmp1 and tmp2){
           if(tmp1->pow > tmp2->pow){
             Node* val = new Node(tmp1->coeff, tmp1->pow);  
             temp->next = val;
             tmp1 = tmp1->next;
           }
           else if(tmp2->pow > tmp1->pow){
             Node* val = new Node(tmp2->coeff, tmp2->pow);  
             temp->next = val;
             tmp2 = tmp2->next;
           }
           else{
             Node* val = new Node(tmp1->coeff + tmp2->coeff, tmp1->pow);  
             temp->next = val;
             tmp1 = tmp1->next;
             tmp2 = tmp2->next;
           }
       ...
       if(tmp1) temp->next = tmp1;
       if(tmp2) temp->next = tmp2;
       return ans->next;
    }
};
